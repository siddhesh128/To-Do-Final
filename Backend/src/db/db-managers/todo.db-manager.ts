...
class TodoDbManager {
  ...
  // Add Todo with optional taskImage
  public static async createTodo(userId: number, task: string, expectedTime?: string, taskImage?: string): Promise<TodoAttributes> {
    try {
      ...
      if (expectedTime !== undefined) {
        newTodo.expectedTime = new Date(expectedTime); // Set the expected time if provided
      }
      if (taskImage !== undefined) {
        newTodo.taskImage = taskImage; // Set the task image if provided
      }
  console.log("Add Todo Service Input:", { userId, task, expectedTime, taskImage });
  ...
  public static async updateTodoTask(todoId: number, userId: number, task: string, taskImage?: string, expectedTime?: string): Promise<boolean> {
    try {
      ...
      if (taskImage !== undefined) {
        updateData.taskImage = taskImage; // Update the task image if provided
      }
      if (expectedTime !== undefined) {
        updateData.expectedTime = new Date(expectedTime); // Update the expected time if provided
      }
